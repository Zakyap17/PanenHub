                ...
                <form action="/api/checkout" method="POST" id="checkoutForm" onsubmit="return submitCheckout()" enctype="multipart/form-data">
                    ...
                    <div class="mb-3">
                        ...
                        <select name="payment_method" id="paymentMethodSelect" class="form-select bg-light border-0 py-2" required onchange="toggleBankInstructions(this.value)">
                            ...
                            <option value="Transfer ke Rekening Mitra">Transfer ke Rekening Mitra</option>
                    ...
                    <div class="mb-3">
                        ...
                        <div id="bankInstructions" class="mb-3 p-3 bg-warning-subtle rounded border border-warning" style="display:none"></div>
                    ...
                    <div id="proofOfPaymentContainer" class="mb-3" style="display:none">
                        ...
                        <div class="border rounded-3 p-3 bg-light text-center">
                            ...
                            <div class="form-text small mt-1">Upload foto atau screenshot bukti transfer Anda.</div>
                        </div>
                    </div>
...
<script>
    ...
    function toggleBankInstructions(val) {
        ...
        if (val === 'Transfer ke Rekening Mitra') {
            ...
            // Generate radio list of banks per mitra in cart
            ...
            mitraNames.forEach(function(m) {
                if (mitraBanks[m] && mitraBanks[m].length > 0) {
                    html += '<h6 class="fw-bold mt-2 text-dark border-bottom pb-1">Rekening ' + m + '</h6>';
                    mitraBanks[m].forEach(function(b) {
                        var radioId = 'bank_' + b.id;
                        html += '<div class="form-check mb-2">';
                        html += '<input class="form-check-input" type="radio" name="bank_option" id="' + radioId + '" value="' + b.id + '" onchange="document.getElementById(\'selectedBankInput\').value = this.value;">';
                        html += '<label class="form-check-label" for="' + radioId + '">';
                        html += '<div class="bank-detail bg-white p-2 rounded border">';
                        html += '<p class="mb-0 small fw-bold text-primary">' + b.bank_name + '</p>';
                        html += '<h5 class="fw-bold mb-0 text-dark">' + b.account_number + '</h5>';
                        html += '<div class="small text-muted">a/n ' + b.account_name + '</div>';
                        html += '</div>';
                        html += '</label></div>';
                    });
                } else {
                    html += '<div class="small text-danger mb-2">Mitra ' + m + ' belum mengatur rekening bank. Silakan pilih metode lain.</div>';
                }
            });
            box.innerHTML = html;
            }
    ...
    document.addEventListener('click', function(e) {
        if (e.target && e.target.id === 'btnCheckout') {
            ...
            mitraNames.forEach(function(m) {
                if (mitraBanks[m] && mitraBanks[m].length > 0) {
                    bankHtml += '<h6 class="fw-bold mt-2 text-dark border-bottom pb-1">Rekening ' + m + '</h6>';
                    mitraBanks[m].forEach(function(b) {
                        bankHtml += '<div class="bank-detail mb-2 bg-white p-2 rounded border">';
                        bankHtml += '<p class="mb-0 small fw-bold text-primary">' + b.bank_name + '</p>';
                        bankHtml += '<h5 class="fw-bold mb-0 text-dark">' + b.account_number + '</h5>';
                        bankHtml += '<div class="small text-muted">a/n ' + b.account_name + '</div>';
                        bankHtml += '</div>';
                    });
                } else {
                    bankHtml += '<div class="small text-danger mb-2">Mitra ' + m + ' belum mengatur rekening bank. Silakan pilih metode lain.</div>';
                }
            });
            bankHtml += '<p class="mt-3 mb-0 x-small text-muted" style="font-size: 0.7rem;">Silahkan transfer ke rekening Mitra untuk memproses pesanan.</p>';
            document.getElementById('bankInstructions').innerHTML = bankHtml;
